...
class AppWindow(Gtk.Window):
    ...
    def execute_processing_pipeline(self, target_dir, theme_name, cursor_sizes_str, build_hypr):
        def task():
    ...
    def on_start_process(self, widget):
        ...
        build_hypr = self.chk_hypr.get_active()

        # Check Hyprcursor dependency ONLY when build_hypr is enabled
        if build_hypr and not engine.check_hyprcursor_installed():
            def install_and_continue():
                def task():
                    res = engine.install_hyprcursor(self.append_log)
                    if res == 0 and engine.check_hyprcursor_installed():
                        self.execute_processing_pipeline(target_dir, theme_name, cursor_sizes_str, True)
                    else:
                        self.append_log("[Error] hyprcursor installation failed.\n")
                threading.Thread(target=task, daemon=True).start()

            def skip_and_continue():
                GLib.idle_add(lambda: self.chk_hypr.set_active(False))
                self.execute_processing_pipeline(target_dir, theme_name, cursor_sizes_str, False)

            self.show_in_app_modal(
                t('hypr_missing_title'),
                t('hypr_missing_desc'),
                [
                    (t('cancel'), None, False),
                    (t('skip_hypr'), skip_and_continue, False),
                    (t('install_hypr'), install_and_continue, True)
                ]
            )
            return

        self.execute_processing_pipeline(target_dir, theme_name, cursor_sizes_str, build_hypr)#!/usr/bin/env python3
import os
import sys
import atexit
import subprocess
